...
import { GetAllNotify } from "../../mocks";
function Notification() {
  ...
  const getNotification = async () => {
    ...
    // setNotificationList(GetAllNotify.data.resultObj);
  };
    ...
    <>
      ...
      <div className={styles.container}>
        <div className={styles.wrapper}>
          <h1>Thông báo</h1>
          <div className="max-h-[52vh] min-h-[52vh] overflow-auto">
            {notificationList
              ? notificationList.map((notify) => (
                  <div className={styles.item_notify} key={notify.id}>
                    <p>{notify.content}</p>
                    <span className={styles.time}>
                      {timeCaculate(notify.dateCreated)}
                    </span>
                  </div>
                ))
              : ""}
