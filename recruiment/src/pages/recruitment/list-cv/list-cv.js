...
import { Form, Button, Table, Modal, Select, message, DatePicker, Tag } from "antd";
...
function ListCV() {
  ...
  const getCVList = async () => {
    await axios
      .get(`https://localhost:5001/api/Companies/GetAllCV?id=${id}`)
      .then((res) => {
        setloading(false);
        setCvList(
    ...
    //   setCvList(
    //     GetAllCV.data.resultObj.map((row, index) => ({
    //       key: index,
    //       dateSubit: GetFormattedDate(row.dateSubit),
    //       filePath: row.filePath,
    //       recruitmentId: row.recruitmentId,
  ...
  const columns = [
    {
      title: "Tên",
      render: (key)=>(
        <span className="font-bold">{key.recruitment}</span>
      )
    },
    {
      title: "Tên ứng viên",
    ...
    },
    {
      ...
      render: (key)=>(
        <Tag color='cyan'>{key.academicLevel}</Tag>
      )
    },
    {
        ...
        return (
          <div className="flex items-center space-x-2">
            ...
            <AiFillCheckCircle
              ...
              style={{ marginLeft: 18, color: "green", fontSize: "1.6rem" }}
            />
            <AiFillCloseCircle
              onClick={() => {
                onRefuseCV(key.recruitmentId, key.userId);
              }}
              style={{ color: "red", marginLeft: 18, fontSize: "1.6rem" }}
            />
          </div>
        );
      },
  ...
  const onDownloadCV = async (filePath) => {
    try {
      await axios
        .get(
          `https://localhost:5001/api/Companies/DownloadCV?fileName=${filePath}`
        )
        .then((res) => {
