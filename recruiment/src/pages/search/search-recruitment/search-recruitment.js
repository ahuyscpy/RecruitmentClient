...
function SearchRecruitment() {
  ...
  const getSearchInformation = async () => {
    ...
    // setTotalRecord(GetAllRecruitmentPaging.data.resultObj.totalRecords);
  };
  const ranks = [
    {
      id: 1,
    ...
    await axios
      .get(
        `https://localhost:5001/api/Companies/GetAllRecruitmentPaging?PageIndex=1&PageSize=10${
          ...
          salary ? "&Salary=" + salary : ""
        }${value ? "&Education=" + value : ""}${type ? "&Type=" + type : ""}${
          branch ? "&BranchId=" + branch : ""
        }${career ? "&CareerId=" + career : ""}`
      )
      .then((res) => {
          ...
          <Row gutter={[30, 30]}>
            {listRecruitments &&
              listRecruitments.map((recruitment) => (
                <Col span={12} key={recruitment.id}>
                      ...
                      <div className="flex items-start justify-between space-x-2">
                        <div className="border-[1px] w-[100px] rounded-[6px] h-[100px] p-2 border-solid">
                          <Image
                            ...
                            preview={false}
                            alt=""
                            src={'https://localhost:5001/avatars/' +recruitment?.avatarPath}
                          />
                        </div>
                        ...
                        <div className="flex flex-col justify-between h-[90px]">
                          ...
                          <div className="flex items-center space-x-1 text-[14px] italic opacity-[0.6]">
                            <MdDateRange className={styles.date_icon} />
                            <span>{timeCaculate(recruitment.dateCreated)}</span>
                          </div>
                        </div>
                      </div>
                    </Card>
